...
<li class="card-hover flex flex-row shadow-2xl rounded-lg box {game.price === '0' ? 'hidden' : ''}">
    <div class="relative aspect-video overflow-hidden sm:aspect-square sm:max-w-[150px] bg-white">
        <img
    ...
    <div class="mt-2 flex-auto flex-col gap-3">
        {#if game?.url?.indexOf('howtoplay') > -1}
            <img src="{HOW_TO_PLAY.logo}"
                 class="w-20 px-1.5 py-0.5"
                 alt="How To Play - suggestions..."/>
            <span class="capitalize text-warning-400-800-token italic ml-20 -mt-10 absolute">{game.retailer.name}</span>
        {:else}
            <img src={game.retailer.logo} alt={game.retailer.name}
                 class="w-24 px-1.5 py-0.5 bg-gray-500 bg-opacity-66 rounded-3xl"/>
        {/if}

        <h2 class="mt-2 text-left font-bold">
            {@html game.name}
        </h2>
        <div class="mt-3 badge bg-violet-400 rounded-full max-w-max text-black text-xl">{@html game.price === '0' ? '?' : game.price } lei</div>
        {#if game?.bggUser}
        ...
        <div class="flex justify-end mt-auto w-full">
            <a
                    ...
                    target="_blank"
                    class="btn btn-md m-4 bg-yellow-400 font-bold rounded-md text-orange-950">Mergi la magazin</a
            >
        </div>
    </div>
