...
export async function GET({ fetch, url }) {
	...
	const shiparea =
		zone === null || ['ro', 'bg', 'gr', 'hu'].indexOf(zone) > -1
			? `country=${zone?.toUpperCase() || 'RO'}`
			: 'shiparea=' + zone;
	...
	if (username) {
		...
		console.log(url);
		console.log(
			`Search for [geekmarket list] on BGG for [${currentUser.name}:${currentUser.id}] [${shiparea}]`
		);

		const response = await fetch(
			`${GEEK_MARKET_API}&userid=${currentUser.id}&stock=instock&productstate=active`
		);
		...
		for (const id of suggestedIds) {
			if (id > 0 && counter-- > 0) {
				...
				if (localResult?.products?.length > 0) {
					allResults = [
						...allResults,
						...localResult.products.filter((p) => parseInt(p.quantity) > 0)
					];
				}
			}
		}
