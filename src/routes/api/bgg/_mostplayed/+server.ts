...
export async function GET({ fetch }) {
	...
	const response1 = await fetch(
		`https://api.geekdo.com/api/listitems?page=1&listid=${currentMonthID}`
	);
	const response2 = await fetch(
		`https://api.geekdo.com/api/listitems?page=2&listid=${currentMonthID}`
	);
	...
	if (items?.length > 0) {
		return json({
			...
			title: `BGG MostPlayed [${currentMonthName} 2025]`,
			data: items
		});
	}
