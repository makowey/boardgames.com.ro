...
import {
	...
	extractShopifyGamesFromHtmlJOCOZAUR,
	extractShopifyGamesFromHtmlLUDICUS,
	promotionCalculator
} from '$lib/utils';
...
export async function GET({ url, fetch }) {
	...
	try {
		if (search?.startsWith('@')) {
			...
			const executionTime: number = Math.abs(
			...
			return json({
				...
				games: games.sort(
					(a, b) => b.promotion - a.promotion || parseFloat(a.price) - parseFloat(b.price)
				),
				executionTime
			});
		}
	...
	if (gameIDs.length > 0) {
		...
		raftulCuJocuri.forEach((result, index) => {
			if (result.status === 'fulfilled') {
				result.value.json().then((boardiacsContent: any) => {
					if (Array.isArray(boardiacsContent)) {
						const dataBoardiacs: Game[] = boardiacsContent
							.map((g) => {
								return {
									...g,
									image: g.thumbnail,
		...
		async ([
						...
						...dataPion.response
							...
							.map((game: Game) => {
								return {
									...
									promotion: game?.special
										? promotionCalculator(parseInt(game.special), parseInt(game.price))
										: 0,
			...
			try {
				if (kritResponse?.value) {
					...
					dataKrit = dataKrit?.data?.products
			...
			try {
				if (guildHallResponse?.value) {
					...
					guildHallContent = guildHallContent
						.split('<\\/style>')?.[1]
						?.split('","games":')[0]
						?.replaceAll('\\n', '')
						?.replaceAll('\\t', '')
			...
			try {
				if (gameologyResponse?.value) {
					...
					gameologyContent = gameologyContent
						.split('<\\/style>')?.[1]
						?.split('","games":')[0]
						?.replaceAll('\\n', '')
						?.replaceAll('\\t', '')
			...
			try {
				if (magazinulDeSahResponse?.value) {
					...
					magazinulDeSahContent = magazinulDeSahContent
						.split('<\\/style>')?.[1]
						?.split('","games":')[0]
						?.replaceAll('\\n', '')
						?.replaceAll('\\t', '')
					...
					dataMagazinuldeSah = extractGoMagGamesFromHtml(
						new JSDOM(magazinulDeSahContent),
						MAGAZINUL_DE_SAH
					);
			...
			if (howToPlay) {
				...
				responseJSON.games.forEach((game) => {
					const index = games.findIndex(
						(g) => game.name === g.name && game.retailer?.logo?.indexOf(g.retailer.site) > -1
					);
			...
			if (search?.split(' ')?.length === 1) {
				...
				games = games.filter(
					(game) => game.name?.toLowerCase().indexOf(search.trim().toLowerCase()) > -1
				);
			}
		}
	);
