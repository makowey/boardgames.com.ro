	...
	for (let i = 0; i < childBoxes.length; i++) {
		...
		games.push({
			...
			promotion: acBox.getElementsByClassName('ac-price')[0]?.getElementsByTagName('s')[0]
				? promotionCalculator(
						parseInt(acBox.getElementsByClassName('text-main')[0]?.innerHTML?.replaceAll('RON')),
						parseInt(
							acBox
								.getElementsByClassName('ac-price')[0]
								?.getElementsByTagName('s')[0]
								.innerHTML?.replaceAll('RON')
						)
				  )
				: 0,
...
export function extractShopifyGamesFromHtmlJOCOZAUR(dom: any, retailer: Retailer) {
	...
	const games: Game[] = [];
	const childBoxes = el
		?.getElementById('predictive-search-results-products-list')
		.getElementsByClassName('predictive-search__list-item');
	for (let i = 0; i < childBoxes?.length; i++) {
		const acBox = childBoxes[i];
		games.push({
...
export function extractFromRedGoblinHtml(dom: any, retailer: Retailer) {
	const rootElement = dom.window.document
		.getElementsByClassName('search-flydown--product-items')[0]
		.getElementsByClassName('search-flydown__group-list')[0];
